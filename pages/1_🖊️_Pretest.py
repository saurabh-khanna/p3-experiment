...
st.markdown('<p style="font-size: 24px;">Before you start using S🎈nder to browse the internet, please answer the following questions to the best of your ability.</p>', unsafe_allow_html=True)
st.write("&nbsp;")

with st.form("pretest"):
    gender = st.selectbox("What gender do you identify as?", ['', "Male", "Female", "Other"])
    st.write("&nbsp;")
    age = st.slider("What is you current age?", 0, 100, 50)
    st.write("&nbsp;")
    ethnicity = st.selectbox("What is your ethnic background?", ['', "Caucasian", "African-American", "Latino or Hispanic", "Asian", "Native American", "Two or more ethnicities", "Other"])
    st.write("&nbsp;")
    education = st.selectbox("What is your highest level of education?", ['', "High School", "Bachelor’s degree", "Master’s degree or above", "Other"])
    st.write("&nbsp;")
    marital_status = st.selectbox("What is your marital status?", ['', "Single", "Married", "Divorced", "Widowed", "Other"])
    st.write("&nbsp;")
    household_income = st.selectbox("What is your annual household income?", ['', "Less than $25,000", "$25,000 to $49,999", "$50,000 to $74,999", "$75,000 to $99,999", "$100,000 to $149,999", "$150,000 or more"])
    st.write("&nbsp;")
    domicile = st.selectbox("Which of these best describes the general area where you live?", ['', "Urban", "Suburban", "Rural", "Other"])
    st.write("&nbsp;")
    political_affiliation = st.selectbox("What is your political affiliation?", ['', "Democrat", "Republican", "Independent", "Other"])
    st.write("&nbsp;")
    religion = st.selectbox("What is your religion?", ['', "Christian", "Jewish", "Muslim", "Hindu", "Buddhist", "Atheist", "Other"])

    st.markdown("----")
    st.markdown("&nbsp;")

    st.warning("Please rate your agreement or disagreement with the following statements.")
    st.markdown("&nbsp;")
